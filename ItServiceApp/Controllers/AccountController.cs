...
namespace ItServiceApp.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            _userManager = userManager; //burda dependency injection yapmış oluyoruz. _userManager field'ına atıyoruz newlemeden. Çünkü new'leyerek tanımlasak çünkü güçlü bağ yapmış oluyoruz.
        ...
        public async Task<IActionResult> Register(RegisterViewModel model)
        {
            ...
            var user = await _userManager.FindByNameAsync(model.UserName); //Async biten metotlar asenkrondur ve asenkron çalışması lazım (await ile yani)
            if (user != null)
            {
            ...
            var result = await _userManager.CreateAsync(user, model.Password); //şifremizi şifreliyor.
            if (result.Succeeded)
            {
                ...
                //kullannıcıya email doğrulama gönderme
        ...
        public async Task<IActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            {
                await _userManager.RemoveFromRoleAsync(user, RoleNames.Passive);//burada mailden aktifleştirdiğimizde hesabı pasifliğini kaldırıyor.
                await _userManager.AddToRoleAsync(user, RoleNames.User);//ve de kullanıcı haline getiriyor artık oluşturduğumuz hesabı.
            }
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            ...
            if (!ModelState.IsValid)
            {
                return View(model);//burda model yazınca içine, doldurduğumuz yerler yine dolu geliyor. Fakat yazmazsak sayfa boş gelecekti ve bu da kullanıcıyı uğraştıracaktı. 
            }
